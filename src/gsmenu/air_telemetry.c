...
lv_obj_t * air_telemetry_msposd_text;
lv_obj_t * air_telemetry_msposd_section;
...
void create_air_telemetry_menu(lv_obj_t * parent) {
    ...
    air_telemetry_msposd_text = create_text(parent, NULL, "MSPOSD Settings", NULL, NULL, false, LV_MENU_ITEM_BUILDER_VARIANT_1);
    air_telemetry_msposd_section = lv_menu_section_create(parent);
    lv_obj_add_style(air_telemetry_msposd_section, &style_openipc_section, 0);
    cont = lv_menu_cont_create(air_telemetry_msposd_section);
    lv_obj_set_flex_flow(cont, LV_FLEX_FLOW_COLUMN);
